    ...
    public partial class LoginPage : System.Web.UI.Page
    {
                        ...
                        using (SqlCommand cmd1 = new SqlCommand("SELECT Role,Full_Name FROM User_tbl WHERE Email_Address = @Email AND Password = @Password", conn))
                        {
                            ...
                            using (SqlDataReader dr = cmd1.ExecuteReader())
                            {
                                if (dr.Read())
                                {
                                    ...
                                    {
                                        Response.Redirect("CampaignCreatorsHomePage.aspx");
        ...
        protected void ResetPassword_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    string query = "SELECT Email_Address FROM User_tbl WHERE Email_Address = @Email AND Status = 'Active'";
                    using (SqlCommand cmd6 = new SqlCommand(query, conn))
                    {
                        ...
                        else
                        {
                            ...
                            using (SHA256 sha256Hash = SHA256.Create())
                            {
                                ...
                                string updateQuery = "UPDATE User_tbl SET Password = @Password WHERE Email_Address = @Email";
        ...
        private static void SendPasswordEmail(string email, string hashedPassword)
        {
            ...
            {
                conn.Open();
                string query = "SELECT Full_Name FROM User_tbl WHERE Email_Address = @EmailId AND Status = 'Active'";
